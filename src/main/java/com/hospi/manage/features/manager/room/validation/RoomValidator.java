package com.hospi.manage.features.manager.room.validation;
...
public class RoomValidator {
    ...
    public void validateUpdate(Room room, RoomEditForm form, Errors errors) {
        ...
        if (!isValidRoomNumber(room.getFloorNumber(), form.roomNumber())) {
            errors.rejectValue("roomNumber", "invalid", "Invalid room number");
        }

        if (!room.getRoomNumber().equals(form.roomNumber()) && roomNumberExists(form.roomNumber())) {
            errors.rejectValue("roomNumber", "duplicate", "Room already exists");
        }
    }
